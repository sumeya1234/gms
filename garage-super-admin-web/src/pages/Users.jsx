...
const ROLE_CONFIG = {
  SuperAdmin: { bg: 'bg-violet-50', text: 'text-violet-600', border: 'border-violet-200/50', label: 'superAdminRole' },
  GarageManager: { bg: 'bg-amber-50', text: 'text-amber-600', border: 'border-amber-200/50', label: 'garageManager' },
  Mechanic: { bg: 'bg-blue-50', text: 'text-blue-600', border: 'border-blue-200/50', label: 'mechanic' },
  Accountant: { bg: 'bg-teal-50', text: 'text-teal-600', border: 'border-teal-200/50', label: 'accountant' },
  GarageOwner: { bg: 'bg-rose-50', text: 'text-rose-600', border: 'border-rose-200/50', label: 'garageOwner' },
...
const StatusDot = ({ status }) => {
  ...
  return (
    <span className={`inline-flex items-center gap-2 text-sm font-medium ${status === 'Active' ? 'text-emerald-600' : 'text-slate-500'}`}>
      ...
      {status === 'Active' ? t('statusActive') : t(status)}
    </span>
  );
}
...
export default function Users() {
  ...
  const roles = ['All', 'SuperAdmin', 'GarageManager', 'GarageOwner', 'Accountant', 'Mechanic', 'Customer'];
    ...
    <div className="flex flex-col gap-6">
      ...
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">
          {loading ? t('loading') : `${users.length} ${t('totalUsersRegistered')}`}
        </p>
      </div>
        ...
        <div className="flex gap-2 flex-wrap">
          {roles.map(r => (
            <button
              ...
              className={`px-5 py-2.5 rounded-full border text-sm font-bold transition-all shadow-sm ${roleFilter === r
                ? 'bg-blue-600 border-blue-600 text-white shadow-md shadow-blue-500/20'
                : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50 hover:border-slate-400'
                }`}
            >
              {r === 'All' ? t('all') : t(ROLE_CONFIG[r].label)}
            </button>
          ))}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center p-12 text-slate-500 bg-white rounded-2xl border border-slate-200 border-dashed">
          ...
          <p className="text-lg font-medium">{search ? t('noUsersMatchSearch') : t('noUsersFound')}</p>
        </div>
      ) : (
            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  {[t('user'), t('emailAddress'), t('phone'), t('role'), t('status'), t('joinedDate')].map(h => (
