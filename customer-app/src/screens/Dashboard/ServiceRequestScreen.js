...
export default function ServiceRequestScreen({ navigation, route }) {
   ...
   const handleSubmit = async () => {
      if (!garage || !garage.id) {
         showAlert(t('Error'), t('Garage details missing. Please select a garage from the Home tab.'), [], 'error');
         return;
      }
      if (!selectedVehicle) {
         showAlert(t('Error'), t('Please select or add a vehicle first.'), [], 'error');
         return;
      }
      if (selectedServices.length === 0) {
         showAlert(t('Error'), t('Please select at least one service to request.'), [], 'error');
         return;
      }
      if (!selectedTime) {
         showAlert(t('Error'), t('Please select a drop-off time.'), [], 'error');
         return;
      }
      if (availability?.isFullyBooked) {
         showAlert(t('Error'), t('The garage is fully booked on this date.'), [], 'error');
         return;
      }
