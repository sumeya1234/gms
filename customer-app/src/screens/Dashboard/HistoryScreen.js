...
export default function HistoryScreen({ navigation }) {
  ...
  const ServiceCard = ({ item }) => (
    <TouchableOpacity style={styles.historyCard} activeOpacity={0.7} onPress={() => {
      ...
      {/* Deposit Payment */}
      {item.DepositAmount > 0 && !item.IsDepositPaid && item.Status !== 'Rejected' && (
        <View style={{ alignItems: 'flex-end', marginTop: 8, padding: 12, backgroundColor: '#fff7ed', borderRadius: 12, borderWidth: 1, borderColor: '#ffedd5' }}>
          <Text style={{ fontSize: 13, fontWeight: 'bold', color: '#9a3412', marginBottom: 4 }}>
            Preservice Deposit Required: {Number(item.DepositAmount).toLocaleString()} ETB
          </Text>

          {paymentLoading === `dep-${item.RequestID}` ? (
            <ActivityIndicator color="#ea580c" />
          ) : showPayOptions === `dep-${item.RequestID}` ? (
            <View style={{ flexDirection: 'row', gap: 6 }}>
              <TouchableOpacity
                style={{ backgroundColor: '#ea580c', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8, gap: 6 }}
                onPress={() => handlePayNow(item, 'Chapa', true)}
              >
                <CreditCard size={14} color="#fff" />
                <Text style={{ color: '#fff', fontSize: 13, fontWeight: 'bold' }}>Pay Deposit (Online)</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={{ backgroundColor: colors.textGray, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8 }}
                onPress={() => setShowPayOptions(null)}
              >
                <X size={14} color="#fff" />
              </TouchableOpacity>
            </View>
            ...
            <TouchableOpacity
              style={{ backgroundColor: '#ea580c', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 10, borderRadius: 10, gap: 8 }}
              onPress={() => setShowPayOptions(`dep-${item.RequestID}`)}
            >
              <Banknote size={16} color="#fff" />
              <Text style={{ color: '#fff', fontSize: 14, fontWeight: 'bold' }}>Pay Deposit Now</Text>
            </TouchableOpacity>
          )}
          <Text style={{ fontSize: 10, color: '#c2410c', marginTop: 6, fontStyle: 'italic' }}>* Work will start after deposit confirmation</Text>
        </View>
      )}

      {/* GRAND TOTAL PAYMENT (Completed) */}
      {item.Status === 'Completed' && (!item.PaymentStatus || item.PaymentStatus === null) && (
        <View style={{ alignItems: 'flex-end', marginTop: 4 }}>
          <Text style={{ fontSize: 13, fontWeight: 'bold', color: colors.textGray, marginBottom: 4 }}>
            Balance: {(Number(item.PartsCost) + Number(item.BaseServicePrice)).toLocaleString()} ETB
          </Text>
          ...
          {paymentLoading === item.RequestID ? (
            ...
            >
              <CreditCard size={14} color="#fff" />
              <Text style={{ color: '#fff', fontSize: 13, fontWeight: 'bold' }}>Pay Balance</Text>
            </TouchableOpacity>
          )}
        </View>
      )}
  ...
  const handlePayNow = async (request, method, isDeposit = false) => {
    setShowPayOptions(null);
    const loadingId = isDeposit ? `dep-${request.RequestID}` : request.RequestID;
    setPaymentLoading(loadingId);
    try {
      let totalAmount = isDeposit
        ? Number(request.DepositAmount)
        : (Number(request.PartsCost) + Number(request.BaseServicePrice));

      if (totalAmount <= 0) {
        Alert.alert("Error", "Payment amount is zero, cannot proceed.");
      ...
      // ... same logic for redirect or cash message ...
      if (method === 'Cash') {
        Alert.alert(
