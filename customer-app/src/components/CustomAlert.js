...
export default function CustomAlert({
    ...
    const getIcon = () => {
        switch (type) {
            case 'info': return <Info size={48} color={colors.accentBlue} />;
            case 'error': return <AlertTriangle size={48} color={colors.error} />;
            case 'confirm': return <HelpCircle size={48} color={colors.yellowBtn} />;
                    ...
                    <View style={[styles.buttonContainer, finalButtons.length > 1 && styles.buttonContainerVertical]}>
                        {finalButtons.map((btn, idx) => (
                            ...
                            >
                                <Text style={[
                                    styles.buttonText,
                                    btn.style === 'cancel' && { color: colors.textGray }
                                ]}>{btn.text}</Text>
                            </TouchableOpacity>
                        ))}
                    </View>
