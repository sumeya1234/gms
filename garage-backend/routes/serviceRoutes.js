...
import { assignMechanic, createService, updateRequestStatus, respondToOffer, completeService, getMyRequests, getGarageRequests, getRequest, updateAssignment, updateStatusById, getMyAssignments, addAssignmentItems, getRequestItems, hideRequest } from "../controllers/serviceController.js";
...
router.put("/:requestId/respond", protect, authorize("Customer"), validate(validateRequestId, "params"), respondToOffer);
router.delete("/my-requests/:requestId", protect, authorize("Customer"), hideRequest);
router.get("/garage/:garageId", protect, authorize("GarageManager", "GarageOwner"), validate(validateGarageId, "params"), getGarageRequests);
