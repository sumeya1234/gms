...
export default function Inventory() {
  ...
  const openAddModal = () => {
    setEditingItem(null);
    setFormData({ itemName: '', quantity: '', unitPrice: '', supplierName: '', supplierContact: '', supplierEmail: '' });
    setFormError('');
    setIsModalOpen(true);
  };
              ...
              <tbody className="text-sm">
                {inventory.map((item) => (
                  <tr key={item.ItemID} className="border-b border-[var(--color-border)] hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-4 text-sm text-gray-500">
                      {item.SupplierName ? (
                        <div>
                          ...
                          {item.SupplierContact && <div className="text-xs">{item.SupplierContact}</div>}
                        </div>
                      ) : (
                    ...
                    <td className="p-4">
                      <span className={`px-2.5 py-1 rounded inline-flex items-center gap-1.5 font-bold text-[10px] uppercase tracking-wider ${item.Quantity >= 10 ? 'bg-green-100 text-green-700' :
                        item.Quantity > 0 ? 'bg-orange-100 text-orange-700 border border-orange-200' : 'bg-red-100 text-red-700'
                        }`}>
                        <div className={`w-1.5 h-1.5 rounded-full ${item.Quantity >= 10 ? 'bg-green-500' :
            ...
            <form onSubmit={handleSubmit} className="p-6" autoComplete="off">
              ...
              <div className="space-y-4">
                ...
                <div className="pt-4 border-t border-gray-100">
                  <h4 className="text-sm font-bold text-gray-700 mb-3">Supplier Information (Optional)</h4>
                  <div className="space-y-4">
                    <div>
